...
async function downloadFiles(urls, downloadDir, batchSize = 10) {
...
const urls = books.map(book => {
  const pageImages = book.pages.map(page => page.image).filter(a => a);
  pageImages.push(book.image)
  return pageImages;
}).flat();
...
// 指定下载目录
const downloadDir = path.join(__dirname, '/../data/images');

// 设置每批次下载数量
downloadFiles(urls, downloadDir, 10)
